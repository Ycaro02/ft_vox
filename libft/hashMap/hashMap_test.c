...
void hashmap_test() {
    ...
    assert(hashmap_get(map, (t_block_pos){10, 11, 12}) == NULL);

    // Add more entries after expansion
    assert(hashmap_set_entry(map, (t_block_pos){10, 11, 12}, ft_strdup("Value4")) == HASHMAP_ADD_ENTRY);
    assert(hashmap_set_entry(map, (t_block_pos){13, 14, 15}, ft_strdup("Value5")) == HASHMAP_ADD_ENTRY);

    // Retrieve values from the hashmap after expansion and verify
    assert(compare_values(hashmap_get(map, (t_block_pos){10, 11, 12}), "Value4"));
    assert(compare_values(hashmap_get(map, (t_block_pos){13, 14, 15}), "Value5"));
...
void hashmap_colision_test() {
    ...
    assert(hashmap_set_entry(map, (t_block_pos){1, 2, 3}, ft_strdup("Value1")) == HASHMAP_ADD_ENTRY);
    assert(hashmap_set_entry(map, (t_block_pos){5, 6, 7}, ft_strdup("Value2")) == HASHMAP_ADD_ENTRY);
    assert(hashmap_set_entry(map, (t_block_pos){9, 10, 11}, ft_strdup("Value3")) == HASHMAP_ADD_ENTRY);
    ...
    assert(hashmap_set_entry(map, (t_block_pos){13, 14, 15}, ft_strdup("Value4")) == HASHMAP_ADD_ENTRY);
    assert(hashmap_set_entry(map, (t_block_pos){17, 18, 19}, ft_strdup("Value5")) == HASHMAP_ADD_ENTRY);
